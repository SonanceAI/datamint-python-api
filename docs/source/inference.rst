...
Keras, Hugging Face, or any third-party framework. It walks through the full journey of
bringing such a model in:

1. **Integrate** -- wrap it so Datamint can call it.
2. **Log & register** -- track it in MLflow, versioned like any other Datamint model.
3. **Deploy** -- run inference on it from the Datamint UI, with no code required per
   prediction.
...
2. Log & Register the Model in MLflow
----------------------------------------
...
3. Deploy: Run Inference Through the UI
------------------------------------------
